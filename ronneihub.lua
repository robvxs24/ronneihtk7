-- ==============================================================================
--  RONNEI HUB - 100% REAL-TIME LIVE BEST PET + SAFE 3D TEXTURE + FULL VN
-- ==============================================================================
...
-- 1. Khởi chạy song song Script Gốc và Lennon Hub (chạy ngầm lấy dữ liệu Live)
...
-- 3. Từ điển dịch tiếng Việt đầy đủ toàn bộ hệ thống
local Dict = {
...
-- 4. Hook các công tắc Toggle trong Menu
...
-- 5. Quản lý thẻ Best Pet & Bảo vệ luồng Live
...
-- Tìm đúng thẻ Best Pet thực sự (loại bỏ hoàn toàn header)
        ...
        for _, gui in ipairs(cont:GetChildren()) do
            if gui:IsA("ScreenGui") and gui.Name ~= "RonneiHub" and not gui:FindFirstChild("RonneiLangModule", true) then
                for _, d in ipairs(gui:GetDescendants()) do
                    -- Chỉ lấy label BEST EGG độc lập, không dính TELEPORT/SYSTEM
...
-- Bảo dưỡng ViewportFrame liên tục để Pet luôn sáng đẹp và camera luôn chuẩn
...
-- Xóa sổ hoàn toàn rác Lennon Hub ngoài màn hình
local function purgeLennonJunk(activeGui)
    local list = {CG}
    if LP and LP:FindFirstChild("PlayerGui") then table.insert(list, LP.PlayerGui) end
    if gethui then table.insert(list, gethui()) end
    ...
    for _, cont in ipairs(list) do
        for _, gui in ipairs(cont:GetChildren()) do
            if gui:IsA("ScreenGui") and gui ~= activeGui and gui.Name ~= "RonneiHub" and not gui:FindFirstChild("RonneiLangModule", true) then
                ...
                for _, d in ipairs(gui:GetDescendants()) do
                    if d:IsA("TextLabel") and (d.Text:find("LENNON") or d.Text:find("TOP 4 SYSTEM") or d.Text:find("TELEPORT") or d.Text:find("Reset GUI")) then
                        hasLennon = true; break
                    end
                end
                if hasLennon then
                    for _, c in ipairs(gui:GetChildren()) do 
                        if c:IsA("GuiObject") and not isInsidePet(c) then c.Visible = false end 
                    end
                    for _, d in ipairs(gui:GetDescendants()) do
                        if d:IsA("GuiObject") and not isInsidePet(d) then
                            local txt = (d:IsA("TextLabel") or d:IsA("TextButton")) and d.Text:upper() or ""
                            local n = d.Name:lower()
                            if txt:find("TOP 4") or txt:find("LENNON") or txt:find("RESET GUI") or txt:find("TELEPORT") or n:find("logo") or n:find("button") then
                                d.Visible = false
                            end
                        end
                    end
...
-- 6. Làm đẹp giao diện (Bỏ qua hoàn toàn thẻ Pet)
            ...
            for _, cont in ipairs(list) do
                for _, child in ipairs(cont:GetChildren()) do
                    ...
                    if isHub then
                        ...
                        pcall(purgeLennonJunk, child)

                        -- Tiêu hủy ngay nếu header Lennon Hub vô tình lọt vào Ronnei Hub
                        for _, desc in ipairs(child:GetDescendants()) do
                            if desc:IsA("TextLabel") and desc.Text:upper():find("LENNON HUB") and not isInsidePet(desc) then
                                ...
                                j.Visible = false; j:Destroy()
                            end

                            pcall(applyVisuals, desc)

                            -- Ghim thẻ Best Pet lên trên Auto Steal
                            if desc:IsA("TextLabel") and (desc.Text == "Tự động trộm" or desc.Text == "Auto Steal") and desc.AbsolutePosition.X > 160 and not isInsidePet(desc) then
                                ...
                                local p = row.Parent
                                if p and card and card.Parent ~= p then
                                    card.Parent, card.Visible, card.Active, card.Draggable = p, true, false, false
                                    ...
                                    -- Vô hiệu hóa nút mũi tên v để không bung cửa sổ TOP 4
                            ...
                            if isInsidePet(desc) then
                                -- THẺ BEST PET: Tuyệt đối không cache text cũ, để Lennon update tên/tiền theo thời gian thực
                                if desc:IsA("TextLabel") and getgenv().RonneiTranslateVN then
                                    local cur = desc.Text
                                    if Dict[cur] then desc.Text = Dict[cur] end
                                end
                            else
                                -- Toàn bộ các phần còn lại của Ronnei Hub
                        ...
                        -- Cập nhật hình ảnh Pet sắc nét liên tục
                        if mountedPetCard then pcall(updateLivePetVisual, mountedPetCard) end

                        -- Thay Avatar Header (Không đụng ảnh Pet)
